...
FIXTURE = ROOT / "tests" / "fixtures" / "external-frameworks" / "asro-governance-compatibility-cases.v1.json"
OUTPUT = ROOT / "reports" / "external-frameworks" / "asro" / "asro-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if native == "verifier_error":
        return "FAIL_CLOSED", "ATTESTATION_VERIFIER_ERROR"
    if native == "attestation_mismatch":
        return "DENY", "GOVERNANCE_STATE_MISMATCH"
    ...
    if not transition["evidence_fresh"]:
        return "FAIL_CLOSED", "STALE_ATTESTATION_EVIDENCE"
    if not transition["governance_state_current"]:
        return "DENY", "GOVERNANCE_STATE_MISMATCH"
    if not transition["witnesses_consistent"]:
        return "DENY", "SELECTIVE_ATTESTATION_DIVERGENCE"
...
def main() -> int:
    ...
    for case in fixture["cases"]:
        ...
        results.append({
            ...
            "observed_stegverse_result": result,
            "observed_failure_class": failure,
            "case_match": match,
        })

    receipt = {
        "schema": "asro_stegverse_governance_compatibility_receipt.v1",
        "framework_id": "asro",
        ...
        "authority_boundary": {
            "attestation_reconciliation_is_stegverse_allow": False,
            "governance_state_attestation_is_action_authority": False,
            "simulation_is_runtime_observation": False,
            "receipt_is_execution_authority": False,
            "general_compatibility_claim_allowed": False,
        },
        "limitations": [
            "No pinned ASRO release, host agent, edge agent, verifier, or attestation package was executed.",
            "The evaluator tests only the authored translation contract.",
            "Independent witnessing remains evidence and does not create standing or execution authority.",
        ],
    }
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(f"ASRO GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
