...
FIXTURE = ROOT / "tests" / "fixtures" / "external-frameworks" / "oscal-governance-compatibility-cases.v1.json"
OUTPUT = ROOT / "reports" / "external-frameworks" / "oscal" / "oscal-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if result == "FAIL":
        return "DENY", "CONTROL_EVIDENCE_VALIDATION_DENIAL"
    if not transition["document_schema_valid"] or not transition["references_resolved"]:
        return "FAIL_CLOSED", "CONTROL_EVIDENCE_UNRESOLVED"
    if not transition["assessment_fresh"]:
        return "FAIL_CLOSED", "STALE_CONTROL_EVIDENCE"
    if not transition["responsible_party_current"] or not transition["delegation_current"]:
...
def main() -> int:
    ...
    for case in fixture["cases"]:
        ...
        results.append({
            ...
            "translation_mapping": "OSCAL validation and assessment records enter as structured control evidence; current policy, delegation, scope, recoverability, and consequence authority remain independently evaluated.",
    ...
    receipt = {
        "schema": "oscal_stegverse_governance_compatibility_receipt.v1",
        "framework_id": "oscal",
        ...
        "limitations": [
            "No pinned OSCAL release or validator was executed.",
            "No source package, schema-validation transcript, transformation output, or independent reconstruction was attached.",
            "This receipt validates only the declared translation and StegVerse governance decision contract."
        ],
        "authority_boundary": {
            "schema_valid_control_evidence_is_execution_authority": False,
            "assessment_result_is_transition_admissibility": False,
    ...
    print(f"OSCAL GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
