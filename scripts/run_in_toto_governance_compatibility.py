...
OUTPUT = ROOT / "reports" / "external-frameworks" / "in-toto" / "in-toto-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if not transition["layout_current"] or not transition["evidence_fresh"]:
        return "FAIL_CLOSED", "STALE_PROVENANCE_EVIDENCE"
    if not transition["functionary_signatures_valid"]:
        return "DENY", "PROVENANCE_VERIFICATION_DENIAL"
...
def main() -> int:
    ...
    for case in fixture["cases"]:
        ...
        results.append({
            ...
            "translation_mapping": "in-toto verification enters as provenance evidence; deployment and consequence authority are evaluated independently.",
    ...
    receipt = {
        "schema": "in_toto_stegverse_governance_compatibility_receipt.v1",
        "framework_id": "in-toto",
        ...
        "limitations": [
            "No pinned in-toto implementation was executed.",
            "No signed layout or link bundle was verified.",
    ...
    print(f"IN-TOTO GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
