...
FREQUENCY_GENERATOR = ROOT / "scripts" / "generate_canonical_workflow_stability_change_frequency_summary.py"
PUBLIC_HISTORY_URL = "https://stegverse-labs.github.io/admissibility-wiki/status/canonical-workflow-frequency-change-stability-change-history.json"
MAX_ENTRIES = 24


def load_previous() -> tuple[list[dict], dict]:
    override = os.getenv("CANONICAL_FREQUENCY_CHANGE_STABILITY_CHANGE_HISTORY_SOURCE")
    if override:
        path = Path(override)
        if path.exists():
    ...
    request = Request(PUBLIC_HISTORY_URL, headers={"User-Agent": "stegverse-stability-change-history-reconciler/1.0"})
    try:
...
def main() -> int:
    if not CURRENT.exists():
        raise SystemExit("current workflow stability-change receipt is missing")

    current = json.loads(CURRENT.read_text(encoding="utf-8"))
    if current.get("manual_tasks_required") != [] or current.get("user_action_required") is not False:
        raise SystemExit("current stability-change receipt violates no-manual boundary")
    if current.get("descriptive_only") is not True:
        raise SystemExit("current stability-change receipt must remain descriptive")
    if current.get("predictive_claim") is not False:
        raise SystemExit("current stability-change receipt must remain non-predictive")
    if current.get("causal_claim_beyond_receipt_fields") is not False:
        raise SystemExit("current stability-change receipt exceeds causal claim boundary")
    ...
    if not isinstance(receipt_id, str) or not receipt_id:
        raise SystemExit("current stability-change receipt_id is missing")
    ...
    payload = {
        "schema": "admissibility_wiki.canonical_workflow_frequency_change_stability_change_history.v0.1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "repository": "StegVerse-Labs/admissibility-wiki",
        "public_endpoint": "/status/canonical-workflow-frequency-change-stability-change-history.json",
        "retention_policy": {
            "maximum_entries": MAX_ENTRIES,
            "deduplication_key": "receipt_id",
            "ordering": "generated_at ascending",
            "supersession": "new stability-change receipts append; duplicate receipt identifiers replace prior copies; bounded retention removes the oldest entry",
        },
        ...
        "latest_change": {
            ...
            "prior_stability_class": latest.get("prior_stability_class"),
            "resulting_stability_class": latest.get("resulting_stability_class"),
        },
        ...
        "non_claims": [
            "Stability-change history records bounded descriptive workflow evidence only.",
            "Unavailable prior public history initializes a new bounded sequence without creating a manual task.",
            "History counts do not predict future stability classes or establish an independent cause.",
            "History retention does not grant release, deployment, execution, or downstream mutation authority.",
        ],
    }
    ...
    print(
        "CANONICAL WORKFLOW FREQUENCY CHANGE STABILITY CHANGE HISTORY: PASS - "
    ...
    if not FREQUENCY_GENERATOR.exists():
        raise SystemExit("workflow stability-change frequency generator is missing")
    ...
    if completed.returncode != 0:
        raise SystemExit("workflow stability-change frequency generation failed")
    return 0
