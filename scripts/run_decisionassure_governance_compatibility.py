...
FIXTURE = ROOT / "tests" / "fixtures" / "external-frameworks" / "decisionassure-governance-compatibility-cases.v1.json"
OUTPUT = ROOT / "reports" / "external-frameworks" / "decisionassure" / "decisionassure-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if native == "verifier_error":
        return "FAIL_CLOSED", "TRACE_VERIFIER_ERROR"
    if native == "deny_corrupt" or not transition["trace_integrity_verified"]:
        return "DENY", "TRACE_CORRUPTION"
    ...
    if not transition["policy_current"]:
        return "DENY", "POLICY_DRIFT"
    if not transition["evidence_fresh"]:
        return "FAIL_CLOSED", "STALE_TRACE_EVIDENCE"
...
def main() -> int:
    ...
    receipt = {
        "schema": "decisionassure_stegverse_governance_compatibility_receipt.v1",
        ...
        "authority_boundary": {
            "trace_continuity_is_stegverse_allow": False,
            "decisionassure_result_is_execution_authority": False,
            "simulation_is_runtime_observation": False,
            "receipt_is_execution_authority": False,
            "general_compatibility_claim_allowed": False,
        },
        "limitations": [
            "No authorized DecisionAssure artifact package, verifier, schema, implementation, or public canonical source was executed.",
            "The evaluator tests only the authored translation contract.",
            "Trace integrity and causal continuity remain evidence inputs and do not create current standing or execution authority.",
        ],
    }
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(f"DECISIONASSURE GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
