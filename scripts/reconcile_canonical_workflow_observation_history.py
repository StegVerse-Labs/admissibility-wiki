#!/usr/bin/env python3
from __future__ import annotations

import json
import os
import subprocess
import sys
from datetime import datetime, timezone
from pathlib import Path
from urllib.error import HTTPError, URLError
from urllib.request import Request, urlopen

ROOT = Path(__file__).resolve().parents[1]
CURRENT = ROOT / "static" / "status" / "canonical-workflow-observation-receipt.json"
HISTORY = ROOT / "static" / "status" / "canonical-workflow-observation-history.json"
HEALTH_GENERATOR = ROOT / "scripts" / "generate_canonical_workflow_health_summary.py"
PUBLIC_HISTORY_URL = "https://stegverse-labs.github.io/admissibility-wiki/status/canonical-workflow-observation-history.json"
MAX_ENTRIES = 24


def load_previous() -> tuple[list[dict], dict]:
    override = os.getenv("CANONICAL_OBSERVATION_HISTORY_SOURCE")
    if override:
        path = Path(override)
        if path.exists():
            payload = json.loads(path.read_text(encoding="utf-8"))
            return list(payload.get("observations", [])), {
                "source": str(path),
                "result": "LOCAL_SOURCE_LOADED",
            }
        return [], {"source": str(path), "result": "LOCAL_SOURCE_MISSING"}

    request = Request(PUBLIC_HISTORY_URL, headers={"User-Agent": "stegverse-observation-reconciler/1.0"})
    try:
        with urlopen(request, timeout=20) as response:
            payload = json.loads(response.read().decode("utf-8"))
            return list(payload.get("observations", [])), {
                "source": PUBLIC_HISTORY_URL,
                "result": "PUBLIC_HISTORY_LOADED",
                "http_status": int(getattr(response, "status", None) or response.getcode()),
            }
    except HTTPError as exc:
        return [], {"source": PUBLIC_HISTORY_URL, "result": "PUBLIC_HISTORY_UNAVAILABLE", "http_status": exc.code}
    except (URLError, TimeoutError, json.JSONDecodeError) as exc:
        return [], {"source": PUBLIC_HISTORY_URL, "result": "PUBLIC_HISTORY_UNAVAILABLE", "error": str(exc)}


def main() -> int:
    if not CURRENT.exists():
        raise SystemExit("current workflow observation receipt is missing")

    current = json.loads(CURRENT.read_text(encoding="utf-8"))
    if current.get("manual_tasks_required") != [] or current.get("user_action_required") is not False:
        raise SystemExit("current observation violates no-manual boundary")

    previous, prior_observation = load_previous()
    by_id: dict[str, dict] = {}
    for observation in previous:
        receipt_id = observation.get("receipt_id")
        if isinstance(receipt_id, str) and receipt_id:
            by_id[receipt_id] = observation

    receipt_id = current.get("receipt_id")
    if not isinstance(receipt_id, str) or not receipt_id:
        raise SystemExit("current observation receipt_id is missing")
    by_id[receipt_id] = current

    ordered = sorted(
        by_id.values(),
        key=lambda item: str(item.get("created_at", "")),
    )[-MAX_ENTRIES:]

    counts = {
        state: sum(1 for item in ordered if item.get("observation_state") == state)
        for state in ("PASS_OBSERVED", "FAIL_CLOSED_OBSERVED", "INCOMPLETE_OBSERVATION")
    }
    latest = ordered[-1] if ordered else current
    payload = {
        "schema": "admissibility_wiki.canonical_workflow_observation_history.v0.1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "repository": "StegVerse-Labs/admissibility-wiki",
        "public_endpoint": "/status/canonical-workflow-observation-history.json",
        "retention_policy": {
            "maximum_entries": MAX_ENTRIES,
            "deduplication_key": "receipt_id",
            "ordering": "created_at ascending",
            "supersession": "new observations append; prior observations remain reconstructable until bounded retention removes the oldest entry",
        },
        "prior_history_observation": prior_observation,
        "latest_observation": {
            "receipt_id": latest.get("receipt_id"),
            "created_at": latest.get("created_at"),
            "observation_state": latest.get("observation_state"),
            "commit": latest.get("commit"),
            "run_id": latest.get("run_id"),
        },
        "state_counts": counts,
        "observations": ordered,
        "manual_tasks_required": [],
        "user_action_required": False,
        "reconciliation_owner": "canonical build-pages job",
        "next_reconciliation": "next repository-owned canonical workflow trigger",
        "authority_granted": False,
        "release_authority_granted": False,
        "downstream_mutation_authority_granted": False,
        "non_claims": [
            "Observation history records bounded workflow evidence only.",
            "An unavailable prior public history initializes a new bounded sequence and does not create a manual task.",
            "History retention does not grant merge, release, deployment, execution, or downstream mutation authority.",
        ],
    }

    HISTORY.parent.mkdir(parents=True, exist_ok=True)
    HISTORY.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(
        "CANONICAL WORKFLOW OBSERVATION HISTORY: PASS - "
        f"entries={len(ordered)} latest={latest.get('observation_state')} prior={prior_observation.get('result')}"
    )

    if not HEALTH_GENERATOR.exists():
        raise SystemExit("workflow health summary generator is missing")
    completed = subprocess.run(
        [sys.executable, str(HEALTH_GENERATOR)],
        cwd=ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        check=False,
    )
    if completed.stdout:
        print(completed.stdout.rstrip())
    if completed.returncode != 0:
        raise SystemExit("workflow health summary generation failed")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
