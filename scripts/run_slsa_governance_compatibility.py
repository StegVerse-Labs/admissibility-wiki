...
OUTPUT = ROOT / "reports" / "external-frameworks" / "slsa" / "slsa-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if result == "FAIL":
        return "DENY", "PROVENANCE_VERIFICATION_DENIAL"
    if not transition["artifact_digest_matches"]:
        return "FAIL_CLOSED", "ARTIFACT_DIGEST_MISMATCH"
    if not transition["provenance_complete"] or not transition["evidence_fresh"]:
        return "FAIL_CLOSED", "STALE_OR_INCOMPLETE_PROVENANCE"
    if not transition["builder_identity_verified"]:
        return "FAIL_CLOSED", "BUILDER_IDENTITY_UNVERIFIED"
...
def main() -> int:
    ...
    for case in fixture["cases"]:
        ...
        results.append({
            ...
            "translation_mapping": "SLSA verification enters as supply-chain provenance evidence; deployment authority and commit-time admissibility are evaluated independently.",
    ...
    receipt = {
        "schema": "slsa_stegverse_governance_compatibility_receipt.v1",
        "framework_id": "slsa",
        ...
        "limitations": [
            "No pinned SLSA provenance producer or verifier was executed.",
            "No provenance bundle or artifact was verified.",
            "This receipt validates only the declared translation and StegVerse governance decision contract."
        ],
        "authority_boundary": {
            "verified_provenance_is_deployment_authority": False,
    ...
    print(f"SLSA GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
