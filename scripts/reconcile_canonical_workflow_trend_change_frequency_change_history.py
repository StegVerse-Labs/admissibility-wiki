...
def load_previous() -> tuple[list[dict], dict]:
    override = os.getenv("CANONICAL_TREND_CHANGE_FREQUENCY_CHANGE_HISTORY_SOURCE")
    if override:
        path = Path(override)
        if path.exists():
    ...
    request = Request(
        PUBLIC_HISTORY_URL,
        headers={"User-Agent": "stegverse-frequency-change-history-reconciler/1.0"},
    )
    try:
        ...
        return [], {
            "source": PUBLIC_HISTORY_URL,
            "result": "PUBLIC_HISTORY_UNAVAILABLE",
...
def main() -> int:
    if not CURRENT.exists():
        raise SystemExit("current workflow frequency-change receipt is missing")

    current = json.loads(CURRENT.read_text(encoding="utf-8"))
    if current.get("manual_tasks_required") != [] or current.get("user_action_required") is not False:
        raise SystemExit("current frequency-change receipt violates no-manual boundary")
    if current.get("descriptive_only") is not True:
        raise SystemExit("current frequency-change receipt must remain descriptive")
    if current.get("predictive_claim") is not False:
        raise SystemExit("current frequency-change receipt must remain non-predictive")
    if current.get("causal_claim_beyond_receipt_fields") is not False:
        raise SystemExit("current frequency-change receipt exceeds causal claim boundary")
    ...
    if not isinstance(receipt_id, str) or not receipt_id:
        raise SystemExit("current frequency-change receipt_id is missing")
    ...
    counts = {
        state: sum(1 for item in ordered if item.get("change_state") == state)
    ...
    payload = {
        "schema": "admissibility_wiki.canonical_workflow_trend_change_frequency_change_history.v0.1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "repository": "StegVerse-Labs/admissibility-wiki",
        "public_endpoint": "/status/canonical-workflow-trend-change-frequency-change-history.json",
        "retention_policy": {
            "maximum_entries": MAX_ENTRIES,
            "deduplication_key": "receipt_id",
            "ordering": "generated_at ascending",
            "supersession": "new frequency-change receipts append; duplicate receipt identifiers replace prior copies; bounded retention removes the oldest entry",
        },
        ...
        "non_claims": [
            "Frequency-change history records bounded descriptive workflow evidence only.",
