...
FIXTURE = ROOT / "tests" / "fixtures" / "external-frameworks" / "nemo-guardrails-governance-compatibility-cases.v1.json"
OUTPUT = ROOT / "reports" / "external-frameworks" / "nemo-guardrails" / "nemo-guardrails-stegverse-governance-compatibility-receipt.json"
...
def evaluate(case: dict) -> tuple[str, str | None]:
    ...
    if native == "runtime_error":
        return "FAIL_CLOSED", "RAIL_RUNTIME_ERROR"
    if native == "rail_block":
        return "DENY", "RAIL_NEGATIVE_RESULT"
...
def main() -> int:
    ...
    receipt = {
        "schema": "nemo_guardrails_stegverse_governance_compatibility_receipt.v1",
        ...
        "authority_boundary": {
            "rail_pass_is_stegverse_allow": False,
            "rail_block_is_universal_policy_denial": False,
            "simulation_is_runtime_observation": False,
            "receipt_is_execution_authority": False,
            "general_compatibility_claim_allowed": False,
        },
        "limitations": [
            "No NeMo Guardrails package, rail configuration, model, retrieval stack, tool integration, or runtime was executed.",
            "The evaluator tests only the authored translation contract.",
            "A rail result remains bounded runtime evidence and does not create standing or execution authority.",
        ],
    }
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(f"NEMO GUARDRAILS GOVERNANCE COMPATIBILITY: {'PASS' if all_match else 'FAIL'} -> {OUTPUT.relative_to(ROOT)}")
