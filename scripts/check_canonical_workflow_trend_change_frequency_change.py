...
GENERATOR = ROOT / "scripts" / "generate_canonical_workflow_trend_change_frequency_change.py"
CURRENT = ROOT / "static" / "status" / "canonical-workflow-trend-change-frequency-summary.json"
PREVIOUS = ROOT / "tmp-frequency-summary-previous.json"
OUT = ROOT / "static" / "status" / "canonical-workflow-trend-change-frequency-change-receipt.json"


def fail(message: str) -> None:
    raise SystemExit(f"CANONICAL WORKFLOW TREND CHANGE FREQUENCY CHANGE: FAIL - {message}")
...
def main() -> int:
    ...
    previous = {
        ...
        "recency_class": "RECENT_CHANGE",
    }
    current = {
        "frequency_class": "FREQUENT_CHANGE_OBSERVED",
        "recency_class": "CURRENT_RECEIPT_CHANGED",
        "evidence": {"latest_change_receipt_id": "change.4", "evaluated_entries": 4},
        "evaluation_scope": {
    ...
    env = dict(os.environ)
    env["CANONICAL_TREND_CHANGE_FREQUENCY_SOURCE"] = str(PREVIOUS)
    try:
        ...
        if data.get("prior_frequency_class") != "ISOLATED_CHANGE_OBSERVED":
            fail("prior frequency mismatch")
        if data.get("resulting_frequency_class") != "FREQUENT_CHANGE_OBSERVED":
            fail("resulting frequency mismatch")
        if data.get("prior_recency_class") != "RECENT_CHANGE":
        ...
        if data.get("public_endpoint") != "/status/canonical-workflow-trend-change-frequency-change-receipt.json":
            fail("public endpoint mismatch")
        print("CANONICAL WORKFLOW TREND CHANGE FREQUENCY CHANGE: PASS - changed_fields=2 manual_tasks=0")
        return 0
    finally:
