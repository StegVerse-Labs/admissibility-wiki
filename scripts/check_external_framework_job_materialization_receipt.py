...
FIXTURE = ROOT / "tests" / "fixtures" / "external-framework-job-materialization-receipt.blocked.json"
...
def main() -> int:
    ...
    if receipt.get("materialization_state") not in ALLOWED_STATES:
        failures.append("unsupported materialization_state")
    if receipt.get("decision") not in ALLOWED_DECISIONS:
        failures.append("unsupported decision")
    ...
    for field in ("source_readiness", "source_execution_plan"):
        ...
        if not bound.get("path"):
            failures.append(f"{field}.path is required")
        if not HEX64.fullmatch(str(bound.get("sha256", ""))):
            failures.append(f"{field}.sha256 must be a lowercase sha256")
    ...
    for field in ("authority_review", "consequence_boundary_review"):
        ...
        if review.get("status") not in {"NOT_STARTED", "PENDING", "PASS", "DENY"}:
            failures.append(f"{field}.status invalid")
        if not isinstance(review.get("evidence_refs"), list):
            failures.append(f"{field}.evidence_refs must be a list")

    if receipt.get("runtime_execution_authorized") is not False:
        failures.append("runtime_execution_authorized must remain false in materialization receipt")
    ...
    if decision == "ALLOW_MATERIALIZATION_ONLY":
        if state != "MATERIALIZED_NOT_EXECUTABLE":
            failures.append("ALLOW_MATERIALIZATION_ONLY requires MATERIALIZED_NOT_EXECUTABLE")
        if not isinstance(runtime_job, dict):
            failures.append("ALLOW_MATERIALIZATION_ONLY requires a non-executable runtime_job object")
    elif runtime_job is not None:
        failures.append("non-materialization decisions must keep runtime_job null")
