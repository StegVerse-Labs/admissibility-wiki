...
def main() -> None:
    standard=load(STANDARD); status=load(STATUS); union=load(UNION)
    if standard.get("schema") != "external_framework_governance_compatibility_testing_standard.v1": fail("unexpected standard schema")
    for layer in {"native_framework_execution","semantic_translation","stegverse_commit_time_evaluation","failure_boundary_evaluation","fresh_runner_replay","page_and_machine_readable_result_publication"}:
        if layer not in set(standard.get("required_layers",[])): fail(f"standard missing layer: {layer}")
    canonical_ids={e.get("record_id") for e in union.get("entries",[]) if e.get("record_id")}
    if len(canonical_ids)!=38 or union.get("counts",{}).get("records")!=38: fail(f"canonical inventory count mismatch: {len(canonical_ids)}")
    records={r.get("framework_id"):r for r in status.get("records",[])}
    if set(records)!=set(CONTRACTS): fail(f"authored contract set mismatch: {sorted(records)}")
    if not set(records).issubset(canonical_ids): fail(f"unknown framework IDs: {sorted(set(records)-canonical_ids)}")
    ...
    for framework_id,(stem,page_name,state) in CONTRACTS.items():
        ...
        data=load(fixture); cases=data.get("cases",[]); families={c.get("family") for c in cases}
        if not runner.exists() or not page.exists(): fail(f"missing runner or page for {framework_id}")
        if len(cases)!=6 or families!=required_families: fail(f"case contract mismatch for {framework_id}: count={len(cases)} families={sorted(families)}")
        record=records[framework_id]
        if record.get("state")!=state: fail(f"invalid state for {framework_id}: {record.get('state')}")
        if record.get("case_count")!=6 or record.get("stegverse_governance_compatibility_observed") is not False: fail(f"invalid count or pre-claim for {framework_id}")
        for case in cases:
            if case.get("expected_stegverse_result") not in {"ALLOW","DENY","ESCALATE","FAIL_CLOSED"}: fail(f"invalid expected result: {framework_id}/{case.get('case_id')}")
    opa_text=OPA_PIPELINE.read_text(encoding="utf-8") if OPA_PIPELINE.exists() else ""
    ...
    for framework_id in ("spiffe-spire","w3c-verifiable-credentials","in-toto","slsa","sigstore","openid-connect","oauth2","w3c-did"):
    ...
    expected_counts={"canonical_records":38,"contract_authored":10,"governance_compatibility_observed":0,"fresh_runner_reproduced":0,"independent_implementation_reproduced":0,"not_started":28}
    ...
    for phrase in ["does not certify","execution authority","general compatibility","policy evidence","identity verification","credential verification","provenance verification","signature verification","authentication","token acceptance","did control"]:
