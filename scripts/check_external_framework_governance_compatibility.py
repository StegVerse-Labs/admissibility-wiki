...
def main() -> None:
    ...
    for framework_id, (stem, page_name, state) in CONTRACTS.items():
        ...
        data = load(fixture); cases = data.get("cases", []); families = {case.get("family") for case in cases}
    ...
    for framework_id in ("spiffe-spire","w3c-verifiable-credentials","in-toto","slsa","sigstore","openid-connect","oauth2","w3c-did","oscal","openlineage","w3c-prov"):
    ...
    expected_counts = {"canonical_records":38,"contract_authored":13,"governance_compatibility_observed":0,"fresh_runner_reproduced":0,"independent_implementation_reproduced":0,"not_started":25}
    ...
    for phrase in ["does not certify","execution authority","general compatibility","policy evidence","identity verification","credential verification","provenance verification","signature verification","authentication","token acceptance","did control","control evidence","lineage visibility","provenance representation"]:
