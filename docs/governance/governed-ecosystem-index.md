# Governed Ecosystem Index

## Purpose

This page is the public navigation index for governed ecosystem transition framing.

## Core pages

- Governed Ecosystem Transitions
- External Frameworks as an Input Class
- Governed Input Classes
- Governed Output Classes
- Governed Transition Map
- Capability Lifecycle
- Ecosystem Capability Status

## Public verification page

The public-facing page for the capability-status change is:

```text
/governance/ecosystem-capability-status
```

## Boundary

This index is a navigation and explanation surface. It does not claim release authorization, operational standing, production authority, live connector installation, or canonical STRP admission.

## Current status

```text
GOVERNED_ECOSYSTEM_INDEX_PRESENT
```
